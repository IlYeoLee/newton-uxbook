...
## 1. 마지막 두 장 전수 검수 — 끝(2026-08-09, 21항목 전부 통과)
한 번 돌려 표로 받는 스크립트를 만들어 두었다. 고칠 곳은 없었다.

    node probe/serve.js 8877
    "/c/Program Files/Google/Chrome/Application/chrome.exe" --new-window \
      "http://127.0.0.1:8877/_audit.html"

서버가 index.html 에 probe/audit.js 를 끼워 내보내고, 브라우저가 POST /REPORT 로
결과를 보내면 표로 찍고 스스로 끝난다(원본 값은 probe/report.json). 28초.
검수 항목을 늘리려면 probe/audit.js 의 archive()/credits() 에 add() 한 줄씩.

메모
- "즉시 5장"은 HTML 기준이고, 첫 layout() 이 ±5칸을 붙여 실제로는 11장이다.
  나머지 31장도 다가올 때 전부 붙는 걸 확인했다(42/42).
- 확대 카드의 앞뒤는 opacity 가 아니라 .cc-inner 의 rotateY 로 갈린다
  (.cc3d-slot 안에서만 opacity 규칙이다). 검수 코드가 여기서 두 번 헛짚었다.
- 합성 포인터로 몰 때 setPointerCapture 는 던진다 — audit.js 의 noCapture() 로
  잠깐 꺼두고 부른다.
- "마우스에 안 붙는지"는 __band.start/move/end 를 감싸 기록해서, 뗀 뒤에 move 가
  더 오는지로 본다.
