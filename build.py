...
# <video> 로 바뀐다(솔루션 파트의 사진은 영상 버전이 있다).
MOBILE_SRC = {
...
def img_tag(name, cls="media"):
    ...
    mattr = f' data-msrc="assets/{m if "." in m else m + ".png"}"' if m else ""
...
MODE_TABLE = [
    ...
    # Boost = 추진, Press = 저항. 표에서 둘의 설명이 서로 바뀌어 있었다
    # (본문과 "안정·추진·저항·저소음" 순서는 처음부터 이쪽이 맞다).
    ("Boost On", "꺼지는 리듬을 다시 앞으로 이어주는 추진 보조 모드", "발뒤꿈치가 지면에서 떨어지고 앞으로 치고 나가는 순간",
     "와이어 장력으로 발목 중심축을 보조해 뒤꿈치를 위로 끌어올림", "다음 발이 더 쉽게 나가고, 후반 리듬이 끊기지 않음"),
    ("Press On", "반복 구간에 일부러 묵직한 저항을 더하는 훈련 모드", "다리를 앞으로 내밀거나 스윙하는 구간",
...
SCENARIO_MEDIA = {
    ...
    # 마지막 장은 영상 뒤에 사진이 한 장 더 온다 — 세탁으로 마무리되는 대목이라
    # 1번 그룹과 같은 "영상 + 사진" 짜임이다.
    6: ["sc6.mp4", "sc6-2.jpg"],   # 오늘의 두근거림을 다음 도전으로
}
